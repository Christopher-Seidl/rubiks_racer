...
int main(int argc, char **argv) {
  ...
  size_t num_scrambles = argc >= 2 ? std::stoi(argv[1]) : 20;
  auto seed = argc >= 3 ? std::stoi(argv[2]) : time(nullptr);
  srand(seed);
  ...
  std::cout << "scrambling cube..." << std::endl;
  for (size_t i = 0; i < num_scrambles; i++) {
    ...
    starting_cube.rotate_face(face, rotation);
  }
  std::cout << std::endl;
